...
// * Find all references to w and r arguments.
// *     In error handling in http handler section and passed as argument to jsonResponse.

// * Replace the actions of writing to w with composing appropriate return value.
// * Replace the actions of reading from r with passing of appropriate function arguments.

// * Find all references to model catalog.
// *     When making some preliminary queries about the requested model and its metadata.
// *     When invoking getText (the closured function) it's passed as argument.
...
func getModelMetaWithText(digestOrName string, lang []language.Tag, isPack bool) (interface{}, error) {
    ...
    // * Replaced the results coming from http request object accessor functions above 
    // * with values coming from dbget config file or command line arguments.

    //dn := getRequestParam(r, "model")
    // get optional language argument and languages accepted by browser
	//rqLangTags := getRequestLang(r, "lang")
	...
	if digestOrName == "" {
		...
		return _, errors.New("Invalid (empty) model digest and name")
	}

	// find model in catalog
	mdRow, ok := theCatalog.ModelDicByDigestOrName(digestOrName)
	if !ok {
		omppLog.Log("Error: model digest or name not found: ", digestOrName)
		//http.Error(w, "Model digest or name not found"+": "+digestOrName, http.StatusBadRequest)
		return _, errors.New("Model digest or name not found"+": "+digestOrName)
	}

	// if language-specific model metadata not loaded then read it from database
	if ok := theCatalog.loadModelText(mdRow.Digest); !ok {
		omppLog.Log("Error: Model text metadata not found: ", digestOrName)
		//http.Error(w, "Model text metadata not found"+": "+digestOrName, http.StatusBadRequest)
		return _, errors.New("Model text metadata not found"+": "+digestOrName)
	}
	...
	if lc == "" && lcd == "" {
		omppLog.Log("Error: invalid (empty) model default language: ", digestOrName)
		//http.Error(w, "Invalid (empty) model default language"+": "+digestOrName, http.StatusBadRequest)
		return _, errors.New("Invalid (empty) model default language"+": "+digestOrName)
	}

	mt, isOk := getText(&theCatalog, digestOrName, &mdRow, lc, lcd)
	if !isOk {
		//http.Error(w, "Model not found"+": "+mdRow.Name+" "+digestOrName, http.StatusBadRequest)
		return _, errors.New("Model not found"+": "+mdRow.Name+" "+digestOrName)
	}
